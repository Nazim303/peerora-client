...
export default function AboutModal({ isOpen, onClose, theme, lang = 'tr' }) {
    ...
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`${theme.panel} w-full max-w-sm p-6 flex flex-col shadow-2xl space-y-4`}>
        ...
        <p className="text-xs opacity-85 text-center leading-relaxed">
          <strong>Peerora</strong> {t.aboutDesc}
        </p>
        ...
        <div className="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center gap-2 text-[11px] text-emerald-700 font-bold">
          ...
          <span>{t.p2pSecurity}</span>
        </div>

        {/* Linkler */}
        <div className="space-y-1.5 pt-1">
          <a
            href={lang === 'tr' ? "https://www.theosdev.web.tr" : "https://www.theosdev.web.tr/en"}
            target="_blank"
            rel="noreferrer"
            className="w-full py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border border-emerald-500/40 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 transition-colors"
          >
